...
class User_model extends MY_Model {
    ...
    public function get_users($where = '',$row = false, $no_of_row = false) {
        $sql = "SELECT U.*, CONCAT_WS(' ',U.name, U.last_name) as user_fullname, UR.name as user_role_name from ".TBL_USERS." U
    ...
    // for get contractors
    public function get_user($where = '',$row = false, $no_of_row = false) {
        $sql = "SELECT U.*, CONCAT_WS(' ',U.name, U.last_name) as user_fullname, UR.name as user_role_name
        from ".TBL_USERS." U
        left JOIN ".TBL_USER_ROLE." UR ON UR.id = U.user_role AND UR.`is_delete` = 0
        /*LEFT JOIN ".TBL_COUNTRIES." c ON c.id = U.country_id AND c.`is_delete` = 0*/
        WHERE U.is_delete = 0 ".$where." GROUP BY U.user_id ";
        $query = $this->db->query($sql);
        if($row){
