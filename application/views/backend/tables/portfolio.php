...
foreach ($rResult as $key => $aRow) {
    ...
    $row[] = '<label class="switch switch-primary" for="customSwitch' . $key . '">
                <input type="checkbox" name="is_active" value="' . $id . '" class="switch-input btn_portfolio_status" id="customSwitch' . $key . '" ' . ($aRow['is_active'] == 1 ? 'checked' : '') . ' >
    ...
    if (check_permission('product', 'can_edit', $this->user_permission)) {
        $option .= '<a href="' . base_url('backend/portfolio/portfolio_update/' . $id) . '" data-toggle="tooltip" title="" class="btn btn-sm btn-icon"><i class="bx bx-edit"></i></a>&nbsp;';
    }
    if (check_permission('product', 'can_delete', $this->user_permission)) {
        $option .= '<a href="javascript:void(0);" data-id="'.$id.'" data-toggle="tooltip" title="" class="btn btn-sm btn-icon delete_portfolio"><i class="bx bx-trash"></i></a>&nbsp';
    }
    $row[] = $option;
    $output['aaData'][] = $row;
}
